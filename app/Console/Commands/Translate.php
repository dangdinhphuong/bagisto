...
class Translate extends Command
{
    ...
    public function getAllAppLangFiles()
    {
        $directory = base_path('packages');
        // Lấy tất cả các tệp trong thư mục lang/en
        $allFiles = File::allFiles($directory);
        // Mảng để lưu trữ đường dẫn của các tập tin app.php
        $appFiles = [];

        // Lặp qua từng tệp
        foreach ($allFiles as $file) {
            // Kiểm tra xem tên tệp có kết thúc bằng "app.php" không
            $filePath = $file->getPathname();
            // Kiểm tra xem tên tệp có kết thúc bằng "en/app.php" không
            if (strpos($filePath, 'src/Resources/lang/en/app.php') !== false) {
                // Nếu có, thêm đường dẫn vào mảng $appFiles
                $appFiles[] = $filePath;
            }
        }
dd($appFiles);
        // Trả về mảng chứa tất cả các đường dẫn đến các tệp app.php trong thư mục lang/en
        return $appFiles;
    }
